...
class BbcodeParser
{
     ...
     */
    public static function parse($text, $user_bbcodeid = null, $user_bbcode_bitfield = null, $attachments = null)
    {
        $phpbbForumUrl = Worker::getData('phpbburl');

        $bbcode_bitfield = '';
        $bbcode_bitfield = $bbcode_bitfield | base64_decode($user_bbcode_bitfield);
        ...
        if ($user_bbcode_bitfield) {
            $bbcode->bbcode_second_pass($text, $user_bbcodeid, $user_bbcode_bitfield);
        }
        ...
        // code is placed in <pre> tag, therefore, newlines shouldn't be changed to <br />
        // reverting back
        preg_match_all('#<pre .*?</pre>#ms', $text, $codeblocks);
        foreach ($codeblocks[0] as $code) {
            $searchFor = $code;
            $replaceTo = str_replace('<br />', "\n", $code);
            $text = str_replace($searchFor, $replaceTo, $text);
        }

        // clearing smileys
        $text = preg_replace('/<img.*?{SMILIES_PATH}.*?alt="(.*?)".*?>/ms',"$1",$text);

        // changing local URL comment style to keep them for parsing
        if (Worker::getData('convertlocalurls')) {
            $text = preg_replace("/<!-- l -->/ms",Worker::$_localUrlPlaceholder,$text); // MAGIC_URL_LOCAL
        }

        // removing all extra HTML comments: comments, attachments, links
        $text = preg_replace("/<!-- .*? -->/ms","",$text);

        if ($attachments) {
            foreach ($attachments as $attach) {
                $searchFor = '<a href="#" class="attachment">'.$attach['real_filename'].'</a>';
                $showComment = $attach['attach_comment'] ? ' ('.$attach['attach_comment'].')' : '';
                $replaceTo = '<a href="'.$phpbbForumUrl.'download/file.php?id='.$attach['attach_id'].'">'.$attach['real_filename'].$showComment.'</a>';
                $before = $text;
                $text = str_replace($searchFor, $replaceTo, $text);

                // if nothing was replaced it means attachment wasn't placed inline, lets attach to the end
                if ($before == $text) {
                    $text .= '<br><br>'.$replaceTo;
                }
            }
        }
        return $text;
...
class bbcode
{
    ...
    function bbcode_cache_init()
    {
        ...
        {
            $this->template_bitfield = new bitfield($this->bbcode_bitfield);
            $this->template_filename = __DIR__.'/bbcode.html';
